...
namespace XkSystem.Code
{
    public class WeChatConfig
    {
        public static string APP_ID
        {
            get
            {
                if (System.Configuration.ConfigurationManager.AppSettings["APP_ID"] != null)
                {
                    return System.Configuration.ConfigurationManager.AppSettings["APP_ID"];
                }
                else
                {
                    return string.Empty;
        ...
        public static string APP_SECRET
        {
            get
            {
                if (System.Configuration.ConfigurationManager.AppSettings["APP_SECRET"] != null)
                {
                    return System.Configuration.ConfigurationManager.AppSettings["APP_SECRET"];
                }
                else
                {
                    return string.Empty;
    ...
    public class WeChatHelper
    {
        ...
        public static Dictionary<string, object> Get(string url, Encoding encoding = null)
        {
            try
            {
                if (encoding == null)
        ...
        public static bool IsWeChatSwitchingIn(string echostr, string signature, string timestamp, string nonce, string token)
        {
            ...
            tmpStr = Code.Common.GetSwcSH1(tmpStr); //FormsAuthentication.HashPasswordForStoringInConfigFile(tmpStr, "SHA1");//进行sha1加密
        ...
        /// 创建 菜单按钮
        /// </summary>
        public static void CreateWeChatMenu(string menuServerPath, string access_token, string appid, string bm)
        {
            using (FileStream fsMenu = new FileStream(menuServerPath, FileMode.Open))
            {
                using (StreamReader srMenu = new StreamReader(fsMenu, Encoding.UTF8))
                {
                    ...
                    //创建菜单
                    Post("https://api.weixin.qq.com/cgi-bin/menu/create?access_token=" + access_token, menu.Replace("$appid$", appid).Replace("$bm$", bm));
                }
            }
        }
