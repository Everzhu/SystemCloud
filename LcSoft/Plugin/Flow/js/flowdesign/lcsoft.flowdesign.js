(function ($) {
    ...
    var setConnections = function (conn, remove) {
        ...
        if (aConnections.length > 0) {
            ...
            $('#XkSystem_process_info').html(s);
        } else {
            $('#XkSystem_process_info').html('');
        }
    ...
    $.fn.Flowdesign = function (options) {
        var _canvas = $(this);
        //右键步骤的步骤号
        _canvas.append('<input type="hidden" id="XkSystem_active_id" value="0"/><input type="hidden" id="XkSystem_copy_id" value="0"/>');
        _canvas.append('<div id="XkSystem_process_info"></div>');
        ...
        if (processData.list) {
            $.each(processData.list, function (i, row) {
                ...
                .mousedown(function (e) {
                    if (e.which == 3) { //右键绑定
                        _canvas.find('#XkSystem_active_id').val(row.id);
        ...
        $(".process-step").live('click', function () {
            _canvas.find('#XkSystem_active_id').val($(this).attr("process_id")),
        ...
        jsPlumb.makeTarget(jsPlumb.getSelector(".process-step"), {
            ...
            beforeDrop: function (params) {
                ...
                var j = 0;
                $('#XkSystem_process_info').find('input').each(function (i) {
        ...
        var Flowdesign = {
            ...
            getActiveId: function () {
                return _canvas.find("#XkSystem_active_id").val();
            },
            copy: function (active_id) {
                if (!active_id)
                    active_id = _canvas.find("#XkSystem_active_id").val();

                _canvas.find("#XkSystem_copy_id").val(active_id);
                return true;
            },
            paste: function () {
                return _canvas.find("#XkSystem_copy_id").val();
            },
            getProcessInfo: function () {
                try {
                    /*连接关系*/
                    var aProcessData = {};
                    $("#XkSystem_process_info input[type=hidden]").each(function (i) {
